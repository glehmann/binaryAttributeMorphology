...
The filters which are manipulating binary images are often prefixed with the word
"Binary", to differenciate the grayscale versions which don't have a prefix. It
...
The label images are often used to represent the connected components of an
...
The image is a collection of those objects, and it also store some values of the image,
like its size, its spacing, etc.
...
No attribute -- excepted the label -- are stored in this class, which can thus be seen as the base class
for the objects with attributes, or which can be used when no attributes
are required.

The {\em itk::LabelObject} class is templated and takes two required template parameters:
\begin{itemize}
...
the computation of all the set of attributes much efficient\footnote{In the early stage of
...
The position in the images are given in index position when the position is the exact position
of a pixel (for example, the position of the maximum value in the feature image) or in physical
position when the position is given at a subpixel resolution (for example, the centroid). In
both case, the position can easily be converted to the other representation with the 
{\em itk::LabelMap::TransformPhysicalPointToIndex()} and {\em itk::LabelMap::TransformIndexToPhysicalPoint()}
methods.
...
The two first template parameters of the {\em itk::AttributeLabelObject} class or the same
than the ones of the{\em itk::LabelObject} class. The third one is the attribute type.
...
{\em double}
  \item {\em CenterOfGravity} is the center of gravity of the object. Its type is
{\em itk::Point< double >}.
...
attributes of the {\em itk::ShapeLabelObject}s, 
  \item and {\em itk::StatisticsLabelMapFilter} to fill the attributes of the {\em
...
a thresholding of a grayscale image, the objects too small or too big to be of interest
...
highest, or with the lowest attribute values.
  ...
  \item The objects can be relabeled, with the {\em
...
S_{i,j} = \begin{cases}
  ...
  \underset{L \in O}\sum \left( p_i.\underset{p \in L}\sum p_0 \right)  & \text{if $i > 0$ and $j = 0$}, \\
  \underset{L \in O}\sum \left( p_j.\underset{p \in L}\sum p_0 \right)  & \text{if $i = 0$ and $j > 0$}, \\
...
\sum_{p \in L} p_0^2  & = & \sum_{i_0=0}^{l_L-1} ( o_0 + s_0 i_0 )^2 \\ \nonumber
                      & = & \sum_{i_0=0}^{l_L-1} ( o_0^2 + s_0^2 i_0^2 + 2 o_0 s_0 i_0) \\ \nonumber
                      & = & l_L.o_0^2 + s_0^2\sum_{i_0=0}^{l_L-1} i_0^2 + 2 o_0 s_0 \sum_{i_0=0}^{l_L-1} i_0 \\ \nonumber
                      & = & l_L.o_0^2 + s_0^2 \left( \frac{(l_L-1)l_L(2l_L-1)}{6} \right) + 2 o_0 s_0 \left( \frac{(l_L-1)l_L}{2} \right) \\ \nonumber
...
S_{i,j} = \begin{cases}
  \underset{L \in O}\sum \left( l_L.p_i.p_j \right)                & \text{if $i > 0$ and $j > 0$}, \\
  \underset{L \in O}\sum \left( p_i.l_L \left( o_0 + \frac{s_0(l_L-1)}{2} \right) \right) & \text{if $i > 0$ and $j = 0$}, \\
  \underset{L \in O}\sum \left( p_j.l_L \left( o_0 + \frac{s_0(l_L-1)}{2} \right) \right) & \text{if $i = 0$ and $j > 0$}, \\
  \underset{L \in O}\sum \left( l_L\left( o_0^2 + s_0 (l_L-1)\left(\frac{s_0(2l_L-1)}{6}  + o_0 \right) \right) \right)            & \text{if $i = j = 0$}.
           \end{cases}
\end{eqnarray}

A constant must be added when both elements of the second order moment are 
the same. This constant represents the normalized second order central moment of a pixel. This is required 
because the measurements are based on discrete pixels rather than continuous values. The normalized 
second order central moment of a pixel with unit length of a binary image can be found as:

$M_{000} = \int_{0}^{\delta_{x}}\int_{0}^{\delta_{y}}\int_{0}^{\delta_{z}}dzdydx = \delta_{x}\delta_{y}\delta_{z} = \Delta$

$M_{100} = \int_{0}^{\delta{x}}\int_{0}^{\delta{y}}\int_{0}^{\delta{z}}x dzdydx = \delta_{y}\delta_{z}\left[\frac{x^2}{2}\right]_{0}^{\delta{x}} = \frac{\delta_{x}\Delta}{2}$

$M_{110} = \int_{0}^{\delta{x}}\int_{0}^{\delta{y}}\int_{0}^{\delta{z}}xy dzdydx = \delta_{z}\left[\frac{x^2}{2}\right]_{0}^{\delta{x}}\left[\frac{y^2}{2}\right]_{0}^{\delta{y}} = \frac{\delta_{x}\delta_{y}\Delta}{4}$

$M_{200} = \int_{0}^{\delta{x}}\int_{0}^{\delta{y}}\int_{0}^{\delta{z}}x^{2} dzdydx = \delta_{y}\delta_{z}\left[\frac{x^3}{3}\right]_{0}^{\delta{x}} = \frac{\delta_{x}^{2}\Delta}{3}$

$\bar{x} = \frac{M_{100}}{M_{000}} = \frac{\frac{\delta_{x}\Delta}{2}}{\Delta} = \frac{\delta_{x}}{2}$
$\bar{y} = \frac{M_{010}}{M_{000}} = \frac{\frac{\delta_{y}\Delta}{2}}{\Delta} = \frac{\delta_{y}}{2}$

$\mu'_{110} = \frac{M_{110}}{M_{000}} - \bar{x}\bar{y} = \frac{\frac{\delta_{x}\delta_{y}\Delta}{4}}{\Delta} - \frac{\delta_{x}}{2}*\frac{\delta_{y}}{2} = \frac{\delta_{x}\delta_{y}}{4} - \frac{\delta_{x}\delta_{y}}{4} = 0$

$\mu'_{200} = \frac{M_{200}}{M_{000}} - \bar{x}^{2} = \frac{\frac{\delta_{x}^{2}\Delta}{3}}{\Delta} - (\frac{\delta_{x}}{2})^{2} = \frac{\delta_{x}^{2}}{3} - \frac{\delta_{x}^{2}}{4} = \frac{\delta_{x}^{2}}{12}$

Thus, the second order cross moment is 0, and the second order moment is 12 . A straightforward analysis 
holds for other dimensions. This explains why it is necessary to add a scalar 1 
12 to each normalized second 
order central moment when the indices are the same dimension and it is not necessary to add anything for 
normalized second order central moments consisting of cross moments. 
...
produces consecutives labels, so a simple {\em for} loop will do the job.
\small \begin{verbatim}
...
itk.auto_progress()
\end{verbatim} \normalsize
Then declare the type we will use, as in {\em C++}.
\small \begin{verbatim}
Dimension = 2
PixelType = itk.UC
...
\end{verbatim} \normalsize
Now swith to the label map representation, and compute the attribute values. This time, the input image is not a label image, but a binary one.
\small \begin{verbatim}
sstats = itk.BinaryImageToStatisticsLabelMapFilter[ImageType, ImageType, LabelMapType].New(th,
    nuclei)
\end{verbatim} \normalsize
we know there are 4 spots in the nubleus, so keep the 4 biggest spots. The other attributes are also usable - we may have chosen to keep the 4 brightest spots for example.
\small \begin{verbatim}
