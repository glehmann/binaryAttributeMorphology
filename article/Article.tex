...
\begin{eqnarray}
Cm_{i,j} = \sum_{p \in D} I(p).p_i.p_j
\end{eqnarray}

where $Cm_{i,j}$ is the central moment, $D$ is the domain of definition of the
image $I$, $I(p)$ is the pixel value of the image at the position $p$,
$0 \leq i < n$, $0 \leq i < n$, $n$ is the image dimension, and $p_i$ is the
physical position on the axis $i$. With binary images, $I(p)$ is either $0$ 
...
Cm_{i,j} & = & \sum_{p \in D} I(p).p_i.p_j \\ \nonumber
         & = & \sum_{p \in O} p_i.p_j \\ \nonumber
         & = & \sum_{l \in O}  \sum_{p \in L} p_i.p_j
\end{eqnarray}

In a line, $p_i$ is a constant if $i > 0$, so

\begin{eqnarray}
Cm_{i,j} = \begin{cases}
  \underset{L \in O}\sum l_L.p_i.p_j                & \text{if $i > 0$ and $j > 0$}, \\
  \underset{L \in O}\sum l_L.p_i.\underset{p \in L}\sum p_0  & \text{if $i > 0$ and $j = 0$}, \\
  \underset{L \in O}\sum l_L.p_j.\underset{p \in L}\sum p_0  & \text{if $i = 0$ and $j > 0$}, \\
...
In order to use those formulae, the physical position has to be expanded in
\begin{eqnarray}
p_j = o_j + s_j i_j
\end{eqnarray}
...
With equations TODO and TODO, its easy to compute the sum of physical positions of the axis $0$:

\begin{eqnarray}
\sum_{p \in L} p_0  & = & \sum_{i_0=0}^{l_L-1} (o_0 + s_0 i_0) \\ \nonumber
                    & = & l_L.o_0 + s_0\sum_{i_0=0}^{l_L-1} i_0 \\ \nonumber
                    & = & l_L.o_0 + s_0\left( \frac{(l_L-1)l}{2} \right)
\end{eqnarray}

where $l_L$ is the length of the line (in pixels), and the the sum of the square of the physical positions of the axis $0$:
...
Finally, central moments are:
\begin{eqnarray}
Cm_{i,j} = \begin{cases}
  \underset{L \in O}\sum l_L.p_i.p_j                & \text{if $i > 0$ and $j > 0$}, \\
  \underset{L \in O}\sum l_L.p_i \left( l_L.o_0 + s_0\left( \frac{(l_L-1)l_L}{2}  \right) \right) & \text{if $i > 0$ and $j = 0$}, \\
  \underset{L \in O}\sum l_L.p_j \left( l_L.o_0 + s_0\left( \frac{(l_L-1)l_L}{2}  \right) \right) & \text{if $i = 0$ and $j > 0$}, \\
  \underset{L \in O}\sum \left( l_L.o_0^2 + s_0^2 \left( \frac{(l_L-1)l_L(2l_L-1)}{6} \right) + 2 o_0 \left( \frac{(l_L-1)l_L}{2} \right) \right)            & \text{if $i = j = 0$}.
           \end{cases}
\end{eqnarray}
...
\begin{eqnarray}
V_n(r) & = {\pi^\frac{n}{2}r^n\over\Gamma(\frac{n}{2} + 1)}
\end{eqnarray}
...
where $R$ is the roundness, $a$ is the measured area of the object, and the $r$ is the radius of an hypersphere with the same volume than the object.
